...
For LLaDa/Dream with **energy-based reranking** (Invariant EBM), use `llada_energy_eval.yaml` or `dream_energy_eval.yaml`. You must download the Qwen AR model first (see **Downloading Qwen AR models** below).
...
### Downloading Qwen AR models (energy-based evaluation)

If you use `llada_energy_eval.yaml` or `dream_energy_eval.yaml`, set `energy.ar_model_path` to a **local directory** where the Qwen model is saved. Download models from Hugging Face to your directory (e.g. `Your Pretrained Model Path`) by running from the project root:

```bash
# Download all supported Qwen AR models (Qwen2-7B, Qwen2.5-7B, Qwen3-8B, Qwen3-4B)
python download_qwen_ar_models.py --output-dir Your Pretrained Model Path

# Or download only specific models
python download_qwen_ar_models.py --output-dir Your Pretrained Model Path --model Qwen2-7B --model Qwen2.5-7B
```

Then in your config set `energy.ar_model_path` to the full path, e.g. `"Your Pretrained Model Path/Qwen2-7B"`. No code changes are required: `from_pretrained()` accepts either a Hugging Face repo id or a local path.

**Decoding speed and parallel degree:** In both normal and energy-based decoding, the number of tokens **unmasked per denoising step** is dynamic (from the schedule: LLaDa uses `get_num_transfer_tokens`, Dream uses the timestep schedule). By default it is the same with or without energy. To make energy-based decoding **decode more tokens in parallel** (fewer steps, faster when each step is costlier), set `energy.energy_unmask_scale` to a value &gt; 1 (e.g. `1.5`). When energy is applied, the code will unmask up to `energy_unmask_scale` times as many tokens that step (capped at the remaining masked count), so you take fewer denoising steps overall.
